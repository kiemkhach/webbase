...
<s:if test="%{!isCreate}">
	<title><s:property value="%{numConfigLab001.siteName}" /></title>
</s:if>
<s:else>
...
<body>
	<div class="main">
		...
		<s:if test="%{!isCreate}"> 
			<a class="" href="<s:url action='createConfigLab001' />" style="margin-left: 15px;"><s:text name="label.create.siteId" /></a>
			<s:if test="%{listAllConfig != null && listAllConfig.size > 0}">
				<form action="selectSiteLab001">
					<s:select class="input-control select" list="listAllConfig"
						listKey="siteId" listValue="siteName" name="siteId"
						value="%{numConfigLab001.siteId}" onchange="this.form.submit()" cssClass="select-tag"/>
				</form>
			</s:if>
		</s:if>
		<input type="hidden" id="message" value="<s:property value='mes'/>" />
		<div class="lab-name">		
			<img src="../img/logos-carrefour.png">
		...
		<div class="config-site">	
			...
			<s:else>		
				<s:if test="%{listAllConfig != null && listAllConfig.size > 0}">
					<div class="sub-leader">
						...
						:
						<s:property value="%{numConfigLab001.siteId}" />
					</div>			
					<div class="unregister-user">
						<form action="unregisterUserForLab001">
					...
					<div class="register-user">
						<form action="registerUserForLab001">
			...
			<s:if test="%{(!isCreate && listAllConfig != null && listAllConfig.size > 0) || isCreate}">
				<form action="configLab001" method="post" enctype="multipart/form-data">	
					<table>
						<s:hidden name="isCreate" value="%{isCreate}" />		
						<s:hidden name="reportName" value="%{numConfigLab001.reportName}" />
						<s:if test="%{!isCreate}">
							<s:hidden name="id" value="%{numConfigLab001.id}" />
							<tr>
								<s:textfield value="%{numConfigLab001.siteId}" name="siteId" key="label.siteId" />
							</tr>
						</s:if>
						<tr>
							<s:textfield value="%{numConfigLab001.siteName}" name="siteName"
								key="label.siteName" size="50" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.siteSuperficies}"
								name="siteSuperficies" key="label.siteSuperficies" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.siteInfo}" name="siteInfo"
								key="label.siteInfo" size="80" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.modulePos}" name="modulePos"
								key="label.modulePos" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleNeg}" name="moduleNeg"
								key="label.moduleNeg" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleReg}" name="moduleReg"
								key="label.moduleReg" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleEclairage}"
								name="moduleEclairage" key="label.moduleEclairage" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleBureau}"
								name="moduleBureau" key="label.moduleBureau" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleBoulangerie}"
								name="moduleBoulangerie" key="label.moduleBoulangerie" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleCvc}" name="moduleCvc"
								key="label.moduleCvc" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleTemperatureIn}"
								name="moduleTemperatureIn" key="label.moduleTemperatureIn" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleTemperatureOut}"
								name="moduleTemperatureOut" key="label.moduleTemperatureOut" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleDrive}"
								name="moduleDrive" key="label.moduleDrive" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleElectric}"
								name="moduleElectric" key="label.moduleElectric" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleGaz}" name="moduleGaz"
								key="label.moduleGaz" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleTemperatureFranis}"
								name="moduleTemperatureFranis"
								key="label.moduleTemperatureFranis" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.moduleEnergyPreviousYear}"
								name="moduleEnergyPreviousYear"
								key="label.moduleEnergyPreviousYear" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.uriIcon}" name="uriIcon"
								key="label.uriIcon" size="50" />
						</tr>
						<tr>
							<s:textfield value="%{numConfigLab001.logo}" name="logo"
								key="label.logo" size="50" />
						</tr>
						<tr>
							<td class="upload-file-label"><s:text name="label.reportName" /></td>
							<td class="upload-file">			
								<input type="text" disabled="true" size="20" value='<s:property value='%{numConfigLab001.reportName}'/>'/>						
							...
							<td>
								<div class="tbl-button" >					
									...
									<s:if test="%{isCreate}">
										...
										href="<s:url action='cancelConfigLab001' />"><s:text name="label.cancel"/></a>
									</s:if>	
									<s:else>
										<a class="button bg-red bg-active-darkRed fg-white" id="delete-subscription"
										href="<s:url action='deleteConfigLab001'>
										<s:param name="id" value="%{numConfigLab001.id}"></s:param>
										<s:param name="siteId" value="%{numConfigLab001.siteId}"></s:param>
										 </s:url>">
