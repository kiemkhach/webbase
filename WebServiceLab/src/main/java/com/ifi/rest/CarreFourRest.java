...
public class CarreFourRest {
	...
	public @ResponseBody String getURIIcon(@RequestParam String siteId) {
		int site = 0;
		try {
		...
		ConfigLab001 configLab = labDAO.getConfigBySite(site);
	...
	public @ResponseBody String getLogo(@RequestParam String siteId) {
		int site = 0;
		try {
			site = Integer.parseInt(siteId);
		} catch (Exception e) {
	...
	/**
	 * Get infomation of carrefour by siteid
	 * 
	 * @param siteId
	 * @return
	 */
	@RequestMapping(value = "getCarreFourBySite", method = RequestMethod.GET)
	public @ResponseBody String getCarreFourBySite(@RequestParam String siteId) {
		...
		// Create Bean
		CarreFourBean carreFourBean = new CarreFourBean();
		carreFourBean.setSiteId(1);

		// // save database
		// ServiceFactory serviceFactory = new ServiceFactory();
		// @SuppressWarnings("static-access")
		// ApplicationContext context = serviceFactory.getAppCtx();
		// ConfigLab001DAO labDAO = context.getBean(ConfigLab001DAO.class);
		...
		if (configLab.getModuleBoulangerie() != null && !"".equals(configLab.getModuleBoulangerie())){
			...
			carreFourBean.setMapBoulangerieCap(mapBoulangerieCap);
			carreFourBean.setBoulangerie(boulangerie);
		}

		// CVC
		Double cvc = null;
		if (configLab.getModuleCvc() != null && !"".equals(configLab.getModuleCvc())){
			cvc = getConsumptionByModule(configLab.getModuleCvc());
			Map<String, Double> mapCvcCap = new HashMap<String, Double>();
			mapCvcCap.put("Cvc", cvc);
			carreFourBean.setMapCvcCap(mapCvcCap);
			carreFourBean.setCvc(cvc);
		}

		// ElectOut
		Double drive = null;
		if (configLab.getModuleDrive() != null && !"".equals(configLab.getModuleDrive())){
			drive = getConsumptionByModule(configLab.getModuleDrive());
			carreFourBean.setDrive(drive);
		}

		// Compteur elec
		Double comElec = null;
		if (configLab.getModuleElectric() != null && !"".equals(configLab.getModuleElectric())){
			comElec = getConsumptionByModule(configLab.getModuleElectric());
			carreFourBean.setCompteurElec(comElec);
		}

		// Compteur gaz
		Double comGaz = null;
		if (configLab.getModuleGaz() != null && !"".equals(configLab.getModuleGaz())){
			comGaz = getConsumptionByModule(configLab.getModuleGaz());
			carreFourBean.setCompteurGaz(comGaz);
		}

		carreFourBean.setLogoPath(PropertiesReader
				.getValue(ConfigEnum.LOGO_PATH));
		carreFourBean.setSiteStr(configLab.getSiteInfo());
		carreFourBean.setSiteName(configLab.getSiteName());

		// Temperate In
		Integer temperateIn = null;
		if (configLab.getModuleTemperatureIn() != null && !"".equals(configLab.getModuleTemperatureIn())){
			temperateIn = getTemperatureByModule(configLab.getModuleTemperatureIn());
			carreFourBean.setTemperateIn(temperateIn);
		}

		// Temperate In
		Integer temperateOut = null;
		if (configLab.getModuleTemperatureOut() != null && !"".equals(configLab.getModuleTemperatureOut())){
			temperateOut = getTemperatureByModule(configLab
					.getModuleTemperatureOut());
			carreFourBean.setTemperateOut(temperateOut);
		}

		// Energy
		carreFourBean.setCurrentYear("2015");
		carreFourBean.setPastYear("2014");
		Double totalCurrentYear = null;

		Double totalElectric = null;
		if (configLab.getModuleElectric() != null && !"".equals(configLab.getModuleElectric())){
			totalElectric = getConsumptionByModuleByYear(
					configLab.getModuleElectric(), 0, 1);
		}
		Double totalGaz = null;
		if (configLab.getModuleGaz() != null && !"".equals(configLab.getModuleGaz())){
			totalGaz = getConsumptionByModuleByYear(
					configLab.getModuleGaz(), 0, 1);
		}
		...
		// 1, 2);
		Integer totalPastYear = Integer.parseInt(configLab.getModuleEnergyPreviousYear());

		Integer superficies = configLab.getSiteSuperficies();
		if (totalCurrentYear != null && superficies != null && superficies != 0) {
			carreFourBean.setCurrentYearEnergy((int)(totalCurrentYear / superficies));
		}
		if (totalPastYear != null && superficies != null && superficies != 0) {
			carreFourBean.setPastYearEnergy(totalPastYear / superficies);
		}
		if (totalPastYear != null && totalPastYear != 0
				&& totalCurrentYear != null) {
			int percent = (int) (((totalCurrentYear - totalPastYear) / (float) totalPastYear) * 100);
			carreFourBean.setDiscount(percent);
		} else {
			carreFourBean.setDiscount(null);
		}

		// Cau hinh dai du lieu trong file config
		PropertiesData pro = new PropertiesData();
		Map<String, Integer> mapRange = pro.loadRangeConfigInt();
		carreFourBean.setIndexPastYear(getIndexOfEnergy(
				carreFourBean.getPastYearEnergy(), mapRange));
		carreFourBean.setIndexCurrentYear(getIndexOfEnergy(
				carreFourBean.getCurrentYearEnergy(), mapRange));

		// set range value
		carreFourBean.setRange_APlus_Min(mapRange
				.get(ConfigEnum.Range_APlus_Min.toString()));
		carreFourBean.setRange_APlus_Max(mapRange
				.get(ConfigEnum.Range_APlus_Max.toString()));

		carreFourBean.setRange_A_Min(mapRange.get(ConfigEnum.Range_A_Min
				.toString()));
		carreFourBean.setRange_A_Max(mapRange.get(ConfigEnum.Range_A_Max
				.toString()));

		carreFourBean.setRange_B_Min(mapRange.get(ConfigEnum.Range_B_Min
				.toString()));
		carreFourBean.setRange_B_Max(mapRange.get(ConfigEnum.Range_B_Max
				.toString()));

		carreFourBean.setRange_C_Min(mapRange.get(ConfigEnum.Range_C_Min
				.toString()));
		carreFourBean.setRange_C_Max(mapRange.get(ConfigEnum.Range_C_Max
				.toString()));

		carreFourBean.setRange_D_Min(mapRange.get(ConfigEnum.Range_D_Min
				.toString()));
		carreFourBean.setRange_D_Max(mapRange.get(ConfigEnum.Range_D_Max
				.toString()));

		carreFourBean.setRange_E_Min(mapRange.get(ConfigEnum.Range_E_Min
				.toString()));
		carreFourBean.setRange_E_Max(mapRange.get(ConfigEnum.Range_E_Max
				.toString()));

		carreFourBean.setRange_F_Min(mapRange.get(ConfigEnum.Range_F_Min
				.toString()));
		carreFourBean.setRange_F_Max(mapRange.get(ConfigEnum.Range_F_Max
				.toString()));

		carreFourBean.setRange_G_Min(mapRange.get(ConfigEnum.Range_G_Min
				.toString()));
		carreFourBean.setRange_G_Max(mapRange.get(ConfigEnum.Range_G_Max
				.toString()));

		carreFourBean.setRange_H_Min(mapRange.get(ConfigEnum.Range_H_Min
				.toString()));
		carreFourBean.setRange_H_Max(mapRange.get(ConfigEnum.Range_H_Max
				.toString()));

		carreFourBean.setRange_I_Min(mapRange.get(ConfigEnum.Range_I_Min
				.toString()));

		ObjectMapper mapper = new ObjectMapper();
		try {
			return mapper.writeValueAsString(carreFourBean);
		} catch (JsonGenerationException e) {
			e.printStackTrace();
		} catch (JsonMappingException e) {
			e.printStackTrace();
	...
	/**
	 * Service to update carre four config module id and site infomation
	 * 
	 * @param siteInfo
	 * @param superficies
	 * @param froidNeg
	 ...
	 * @return
	 */
	@RequestMapping(value = "configCarreFour", method = RequestMethod.GET)
	public @ResponseBody String configCarreFour(@RequestParam String siteId,
			@RequestParam(required = false) String siteInfo,
			@RequestParam(required = false) String siteName,
			@RequestParam(required = false) String superficies,
			@RequestParam(required = false) String froidNeg,
			@RequestParam(required = false) String froidPos,
			@RequestParam(required = false) String froidRes,
			@RequestParam(required = false) String eclairage,
			@RequestParam(required = false) String bureau,
			@RequestParam(required = false) String boulangerie,
			@RequestParam(required = false) String cvc,
			@RequestParam(required = false) String temperatureInt,
			@RequestParam(required = false) String temperatureOut,
			@RequestParam(required = false) String drive,
			@RequestParam(required = false) String computerElectric,
			@RequestParam(required = false) String computerGaz,
			@RequestParam(required = false) String franis,
			@RequestParam(required = false) String energyPastYear,
		...
		ConfigLab001 configLab = labDAO.findBySite(siteNumber);
	 ...
	 * @return
	 */
	@RequestMapping(value = "configLab001", method = RequestMethod.GET)
	public @ResponseBody String configLab001(@RequestParam String siteId,
			@RequestParam(required = false) String siteInfo,
			@RequestParam(required = false) String siteName,
			@RequestParam(required = false) String superficies,
			@RequestParam(required = false) String froidNeg,
		...
		if (temperatureInt != null) {
			if (!temperatureInt.trim().isEmpty()) {
				...
				if (number != null) {
					...
					return "temperatureInt not exists module: "
							+ temperatureInt;
				}
			} else {
		...
		if (temperatureOut != null) {
			if (!temperatureOut.trim().isEmpty()) {
				...
				if (number != null) {
					...
					return "temperatureOut not exists module: "
							+ temperatureOut;
				}
			} else {
		...
		if (computerElectric != null) {
			if (!computerElectric.trim().isEmpty()) {
				...
				if (number != null) {
					...
					return "computerElectric not exists module: "
							+ computerElectric;
				}
			} else {
	...
	public @ResponseBody String configRangeEnergy(
			@RequestParam String aPlusMax, @RequestParam String aMin,
			@RequestParam String aMax, @RequestParam String bMin,
			@RequestParam String bMax, @RequestParam String cMin,
			@RequestParam String cMax, @RequestParam String dMin,
			@RequestParam String dMax, @RequestParam String eMin,
			@RequestParam String eMax, @RequestParam String fMin,
			@RequestParam String fMax, @RequestParam String gMin,
			@RequestParam String gMax, @RequestParam String hMin,
			@RequestParam String hMax, @RequestParam String iMin) {
	...
	public @ResponseBody String getReportLink(@RequestParam String siteId) {
		...
		String reportLink = PropertiesReader.getValue(ConfigEnum.PDF_BACKLOG_LINK) 
	...
	public @ResponseBody String getToken(@RequestParam String username,
	...
	private Double getConsumptionByModule(String moduleArr) {
		...
		for (int i = 0; i < moduleIdArr.length; i++){
			Integer[] result_tmp = getConsumptionByTime(moduleIdArr[i], Calendar.DAY_OF_YEAR,
					0, PropertiesReader.getValueInt(ConfigEnum.DAY_GET_CONSUMPTION));
			if (result_tmp != null && result_tmp.length > 1) {
//				Integer[] consumpByTime = getConsumptionByTime(moduleIdArr[i], Calendar.DAY_OF_YEAR, 0,
//						PropertiesReader
//								.getValueInt(ConfigEnum.DAY_GET_CONSUMPTION));
//				if (consumpByTime != null && consumpByTime.length > 1){
...
//		return result;
	...
	private Integer getTemperatureByModule(String moduleID) {
		...
		cal.add(Calendar.HOUR_OF_DAY,
				-PropertiesReader.getValueInt(ConfigEnum.HOUR_GET_TEMPERATURE));
		String startHour = String.valueOf(cal.get(Calendar.HOUR_OF_DAY));
		String startDate = sdf.format(cal.getTime());
...
//		}
//		if (agregatLst.size() > 1) {
//			if (agregatLst.get(1).getDateHeureAgregat()
...
//					.compareTo(agregatLst.get(1).getDateHeureAgregat()) == 0) {
//				if (agregatLst.get(1).getHeureAgregat()
//						.compareTo(agregatLst.get(1).getHeureAgregat()) > 0) {
//					result = agregatLst.get(1).getTemperature();
//				}
//			}
//		}
//		return result;
	...
	private int getIndexOfEnergy(Integer energy, Map<String, Integer> mapRange) {
		if (energy == null) {
			return 0;
		}
		int index = 0;
		if (energy >= mapRange.get(ConfigEnum.Range_APlus_Min.toString())
				&& energy <= mapRange
						.get(ConfigEnum.Range_APlus_Max.toString())) {
			index = 1;
		} else if (energy >= mapRange.get(ConfigEnum.Range_A_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_A_Max.toString())) {
			index = 2;
		} else if (energy >= mapRange.get(ConfigEnum.Range_B_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_B_Max.toString())) {
			index = 3;
		} else if (energy >= mapRange.get(ConfigEnum.Range_C_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_C_Max.toString())) {
			index = 4;
		} else if (energy >= mapRange.get(ConfigEnum.Range_D_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_D_Max.toString())) {
			index = 5;
		} else if (energy >= mapRange.get(ConfigEnum.Range_E_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_E_Max.toString())) {
			index = 6;
		} else if (energy >= mapRange.get(ConfigEnum.Range_F_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_F_Max.toString())) {
			index = 7;
		} else if (energy >= mapRange.get(ConfigEnum.Range_G_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_G_Max.toString())) {
			index = 8;
		} else if (energy >= mapRange.get(ConfigEnum.Range_H_Min.toString())
				&& energy <= mapRange.get(ConfigEnum.Range_H_Max.toString())) {
			index = 9;
		} else if (energy >= mapRange.get(ConfigEnum.Range_I_Min.toString())) {
			index = 10;
		}
		return index;
	}
